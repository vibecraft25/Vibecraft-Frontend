...
import { chatSessionManager } from "../server/chat-session-manager.js";
import { mcpClientManager } from "../server/mcp-client.js";
...
export class VibeCraftWebSocketServer {
  ...
  private setupExpress(): void {
    ...
    // 연결 정보 조회 엔드포인트
    this.app.get("/connections", (_req, res) => {
  ...
  private setupWebSocketEvents(connection: WebSocketConnection): void {
    ...
    socket.on("close", (code: number, reason: Buffer) => {
      console.log(
        `[WS] 연결 종료: ${connectionId} (코드: ${code}, 이유: ${reason.toString()})`
      );
  ...
  private async handleJoinChat(
    ...
    try {
      ...
      connection.userId = message.userId || undefined;
      ...
      console.log(
        `[WS] 채팅 참여 완료 (연결: ${connection.id}, 세션: ${sessionId})`
      );
    } catch (error) {
  ...
  private async handleChatMessage(
    ...
    try {
      // 메시지를 세션에 추가
      const chatMessage = await chatSessionManager.addMessage(
        sessionId,
        content,
        "user",
        userId
  ...
  private broadcastToSession(
    ...
    console.log(
      `[WS] 세션 ${sessionId}에 브로드캐스트 (연결: ${sessionConnections.length}개)`
    );
  ...
  public start(): Promise<void> {
    return new Promise((resolve) => {
      this.server.listen(this.config.port, () => {
        ...
        console.log(
          `[WS] WebSocket 서버: ws://localhost:${this.config.port}/ws`
        );
        console.log(
          `[WS] 헬스 체크: http://localhost:${this.config.port}/health`
        );
        resolve();
      });
    });
