...
import { MessageSquare, User, Bot, Target, Database, Wrench, Rocket } from "lucide-react";
...
const ChatView = ({
  ...
  // ProcessStatus 표시 컴포넌트
  const getProcessStatusDisplay = (status: ProcessStatus) => {
    const statusConfig = {
      TOPIC: { icon: Target, text: "주제 설정", color: "#1890ff" },
      DATA: { icon: Database, text: "데이터 수집", color: "#52c41a" },
      BUILD: { icon: Wrench, text: "대시보드 구축", color: "#fa8c16" },
      DEPLOY: { icon: Rocket, text: "배포", color: "#722ed1" },
    };
    ...
    return (
      <div className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg border-l-4" 
  ...
  if (messages.length === 0 && !isLoading) {
    ...
    const isNewChat = sessionState === "IDLE";
    ...
    <div className={`flex flex-col h-full ${className}`} style={{ maxHeight }}>
      ...
      {processStatus && (
        <div className="p-4 border-b border-gray-100">
          {getProcessStatusDisplay(processStatus)}
        </div>
      )}
      ...
      >
        {messages.map((message, idx) => (
          <div
            key={`ChatView-${sessionId}-Chat-${idx}`}
          ...
          >
            ...
            <div className={`min-w-0 max-w-[75%] ${
              ...
              >
                ...
                {message.type === "server" && (message as any).sequence && (message as any).total && (
